...
class Intervento(gismodels.Model):
    ...
    bacino_idrografico = gismodels.ForeignKey(BaciniAll, related_name='bacini_grosseto', db_column='bacidro', help_text='Bacino idrografico L 183/1989')
    
    # Comprensorio LR 79/2012
    comprensorio = gismodels.ForeignKey(ComprensorioAll, related_name='comprensori_grosseto', help_text='Comprensorio LR 79/2012')
    
    #provincia = gismodels.CharField(max_length=2, choices=PROVINCIA_CHOICES, default='AR', blank=False)
    provincia = gismodels.ForeignKey(ProvinceAll, related_name='provincia_grosseto')

    comune = gismodels.ForeignKey(ComuniAll, related_name='comune_grosseto')
    
    # Localita (testo libero)
    localita = gismodels.TextField(blank=True)
    
    # Corso d'acqua
    corso_acqua = gismodels.ForeignKey(ArchiGr, blank=True, null=True, db_column='corsoacqua')
    ...
    fenomeno = gismodels.ForeignKey(FenomenoAll, related_name='fenomeno_grosseto')
    ...
    studio_intervento = gismodels.ForeignKey(StudioInterventoAll, verbose_name="Progetto/Studio o Intervento", related_name='studio_intervento_grosseto', default='02')    
    ...
    tipologia_intervento = gismodels.ForeignKey(TipologiaInterventoAll, related_name='tipologia_intervento_grosseto', blank=True, null=True) 
    ...
    tipo_di_opera = gismodels.ForeignKey(TipoOperaAll, related_name='tipo_opera_grosseto', blank=True, null=True)     
    ...
    categoria_idr_opera = gismodels.ForeignKey(CategoriaOperaAll, verbose_name="Categoria idraulica dell'opera", db_column='cat_idr_op', related_name='categoria_opera_grosseto', blank=True, null=True) 
    ...
    # Ente proponente
    ente_proponente = gismodels.ForeignKey(EnteProponenteAll, related_name='ente_proponente_grosseto', db_column='ente_prop', verbose_name='Ente proponente', help_text='Ente Proponente')
    ...
    ente_attuatore_competente = gismodels.ForeignKey(EnteAttuatoreAll, related_name='ente_attuatore_grosseto', verbose_name='Ente attuatore competente ai sensi della normativa vigente', db_column='ente_attuat', help_text='Ente attuatore competente ai sensi della normativa vigente.<BR/><b><a style="color:#FF0000">Compilazione riservata Regione Toscana.</a></b>', blank=True, null=True)
