...
olwidget.Map = OpenLayers.Class(OpenLayers.Map, {
    ...
    initMap: function(mapDivId, opts) {
        ...
        for (var i = 0; i < layers.length; i++) {
            if (layers[i].name != "Reticolo di gestione DCR 57/2013" && layers[i].name != "Comuni" && layers[i].name != "Province" && layers[i].name != "Italia" && layers[i].name != "Regione Toscana" && layers[i].name != "Anno 2013 col - AGEA" && layers[i].name != "CTR 1:10000") {
                var control;
                control = new OpenLayers.Control.WMSGetFeatureInfo({
                    ...
                    eventListeners: {
                        "getfeatureinfo": function (e) {
                            ...
                            var popup;
                            if (!(popupKey in mapGeo.popupCache)) {
                                    ...
                                    listeners: {
                                        close: (function (key) {
                                            return function (panel) {
                                                delete mapGeo.popupCache[key];
                                            };
                                        })(popupKey)
                                    }
                                });
                                for (var oldkey in mapGeo.popupCache) {
                                    mapGeo.popupCache[oldkey].close();
                                }
                                mapGeo.popupCache[popupKey] = popup;
                            } else {
                                popup = mapGeo.popupCache[popupKey];
                            }
                            var match = e.text.match(/<body[^>]*>([\s\S]*)<\/body>/);
...
olwidget.EditableLayerSwitcher = OpenLayers.Class(OpenLayers.Control.LayerSwitcher, {
    ...
    stopEditing: function() {
        ...
        // TODO: i18n
        this.maximize.innerHTML = "Start Editing";
    ...
    setEditing: function(layer) {
        ...
        // TODO: i18n
        this.maximize.innerHTML = "Stop Editing \"" + layer.name + "\"";
    ...
    loadContents: function() {
        ...
        OpenLayers.Event.observe(this.maximize, "mouseover",
            OpenLayers.Function.bindAsEventListener(this.deactivateDraw, this)
        );        
        OpenLayers.Event.observe(this.maximize, "mouseout",
            OpenLayers.Function.bindAsEventListener(this.activateDraw, this)
        );         
    ...
    deactivateDraw: function(e) {
        var draw = this.map.getControlsByClass('OpenLayers.Control.DrawFeature');
        if(typeof(draw[0]) != 'undefined'){
            draw[0].deactivate();  
        }
    },
    activateDraw: function(e) {
        var draw = this.map.getControlsByClass('OpenLayers.Control.DrawFeature');
        var nav = this.map.getControlsByClass('OpenLayers.Control.PanZoom');
        if(typeof(draw[0]) != 'undefined'){
            if(this.maximize.innerHTML != "Start Editing"){        
                draw[0].activate();
            }
        }  
    },    
...
Ext.onReady(function () {
    ...
    for (var i = 0; i < olwidget.Map.layers.length; i++) {
        //if (layers[i].name != "CTR 1:10000" && layers[i].name != "Comprensori ai sensi della L.R.79/2012" && layers[i].name != "Comuni" && layers[i].name != "Province" && layers[i].name != "Specchi d'acqua") {
            var control;
            control = new OpenLayers.Control.WMSGetFeatureInfo({
                autoActivate: true,
                infoFormat: "text/html",
